...
class VYSE60(BaseJMAParser):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.data_type = "VYSE60" # このパーサーが扱うデータタイプ

    def parse(self, xml_tree, namespaces, data_type_code, test=False):
        """
        地震情報 (VYSE60) のXMLを解析します。
        """
        ...
        parsed_data['body_text'] = self._get_text(xml_tree, '//jmx_ib:Body//jmx_ib:Text/text()', namespaces) + self._get_text(xml_tree, '//jmx_ib:Body//jmx_ib:Appendix/text()', namespaces)
        return parsed_data
